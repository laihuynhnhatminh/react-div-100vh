        ...
        <div style={{ flex: 'auto', padding: '1rem', overflow: 'scroll' }}>
          ...
          >
            <input
              ...
              name="divType"
              id="Div100vh"
              checked={divType === 'Div100vh'}
            />
            <label htmlFor="Div100vh">
              The <code>&lt;Div100vh&gt;</code> React component
            </label>
          </div>
          <div
            onClick={() => {
              setDivType('regularDiv')
            }}
          >
            <input
              type="radio"
...
function useDivType() {
  const divTypeLocalStorage = window.localStorage.getItem('divType') as
    | 'regularDiv'
    | 'Div100vh'
  const [divType, setDivType] = useState<'regularDiv' | 'Div100vh'>(
    divTypeLocalStorage ?? 'Div100vh'
  )
  if (divTypeLocalStorage !== divType)
    window.localStorage.setItem('divType', divType)
  return [divType, setDivType] as const
}

function RegularDiv({ children }: React.PropsWithChildren<unknown>) {
  return <div style={{ ...style, height: '100vh' }}>{children}</div>
}
